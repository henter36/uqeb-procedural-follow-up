...
public static class InstitutionalReportXlsxExporter
{
    ...
    private static void AddFindingsSheet(XLWorkbook workbook, InstitutionalReportModel model)
    {
        ...
        ws.RightToLeft = true;
        var headers = new[] { "الكود", "العنوان", "الوصف", "الدليل", "الخطورة", "النطاق" };
        WriteHeaders(ws, headers);
        var row = 2;
    ...
    private static void AddCriticalCasesSheet(XLWorkbook workbook, InstitutionalReportModel model)
    {
        ...
        ws.RightToLeft = true;
        var headers = new[] { "المعرف", "رقم الوارد", "الموضوع", "الإدارة", "الجهة", "الأولوية", "العمر", "أيام التأخر", "السبب", "الإجراء", "الخطورة" };
        WriteHeaders(ws, headers);
        var row = 2;
    ...
    private static void AddPrioritiesSheet(XLWorkbook workbook, InstitutionalReportModel model)
    {
        ...
        ws.RightToLeft = true;
        var headers = new[] { "الأولوية", "الإجمالي", "مفتوحة", "متأخرة", "متوسط العمر", "ضمن المهلة" };
        WriteHeaders(ws, headers);
        var row = 2;
    ...
    private static void AddDataQualitySheet(XLWorkbook workbook, InstitutionalReportModel model)
    {
        ...
        ws.RightToLeft = true;
        var headers = new[] { "الكود", "الملاحظة", "العدد", "النسبة", "الخطورة", "الحقول", "التصحيح" };
        WriteHeaders(ws, headers);
        var row = 2;
    ...
    private static void AddActionPlanSheet(XLWorkbook workbook, InstitutionalReportModel model)
    {
        ...
        ws.RightToLeft = true;
        var headers = new[] { "الأولوية", "النتيجة", "الإجراء", "المسؤول", "المدة المقترحة", "الحالة", "الدليل" };
        WriteHeaders(ws, headers);
        var row = 2;
