...
public static class FollowUpLetterPrintViewRenderer
{
    ...
    public const string OfficialLetterCss =
        ...
        // A4 page container — min-height allows content overflow for multi-page letters
        ...
        // Header (43mm) — contains basmala, logo and agency info via absolute positioning
        ".official-letter-header{position:relative;height:43mm;}" +
        // Basmala — absolute center top of header
        ".official-letter-basmala{position:absolute;top:5mm;left:0;right:0;text-align:center;font-size:13pt;line-height:1.1;white-space:nowrap;}" +
        // Logo wrap — centered below basmala
        ".official-letter-logo-wrap{position:absolute;top:12.8mm;left:0;right:0;display:flex;justify-content:center;align-items:flex-start;}" +
        // Logo — sized to match reference PDF (43.7mm × max 28.5mm, aspect ratio preserved)
        ".letter-logo{width:43.7mm;max-width:43.7mm;height:auto;max-height:28.5mm;object-fit: contain;display:block;print-color-adjust:exact;-webkit-print-color-adjust:exact;}" +
        // Agency info — absolute top-right of header
        ".official-letter-agency{position:absolute;top:5mm;right:18mm;width:46mm;text-align:center;font-size:8pt;line-height:1.25;}" +
        ".official-letter-agency>div{font-weight:700;}" +
        // Office sub-line (e.g. المتابعة الإجرائية) — slightly smaller
        ".official-letter-office{font-size:7pt;margin-top:1mm;font-weight:400;}" +
        // Decorative frame — absolute, starts at 43.1mm from top, rounded corners (ref PDF: ~24mm radius)
        ".official-letter-frame{position:absolute;top:43.1mm;right:8.0mm;left:8.3mm;bottom:10.2mm;" +
            "border:0.75pt solid #111;border-radius:24mm;background:#fff;box-sizing:border-box;" +
            "pointer-events:none;z-index:1;overflow:visible;}" +
        // Body content — sits in document flow after the header (at ~43mm), padded to appear inside the frame
        ".official-letter-body{position:relative;z-index:2;padding:14mm 16mm 16mm 16mm;direction:rtl;text-align:right;box-sizing:border-box;}" +
